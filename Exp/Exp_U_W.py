...
from Data.Data_loader import Dataset_database_U_W as Dataset_database
from torch.utils.data import DataLoader
from Framework.HiSTGNN_U_W import Model_layer as HiSTGNN_U_W

class Exp_Model(Exp_Basic):
    def __init__(self, args,con):
        super(Exp_Model, self).__init__(args)
        self.con = con
        self.rs_12, self.rr_12, self.rs_23, self.rr_23, self.r_type_12, self.r_type_23 = self._get_graph_paras()
        self.model = self._build_model().to(self.device)

    def _build_model(self):
        model_dict = {
            'HiSTGNN_U_W': HiSTGNN_U_W,
        }
    ...
    def _get_graph_paras (self):
        ...
        return rs_12, rr_12, rs_23, rr_23, r_type_12, r_type_23
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        (rs_12, rr_12, rs_23, rr_23) = (self._dim_expansion(x, batch_size) for x in
                                        (self.rs_12, self.rr_12, self.rs_23, self.rr_23))
        self.model.eval()
        total_loss = []
        for i, (batch_en_att_1,batch_en_att_2,batch_en_att_3,batch_label) in enumerate(vali_loader):
            pred = self.model(batch_en_att_1,batch_en_att_2,batch_en_att_3,rs_12,rr_12,
                              rs_23,rr_23,self.r_type_12,self.r_type_23)
    ...
    def tes(self, tes_data, tes_loader, criterion, logger_train):
        ...
        self.model.eval()
        total_loss = []
        for i, (batch_en_att_1,batch_en_att_2,batch_en_att_3,batch_label) in enumerate(tes_loader):
            pred = self.model(batch_en_att_1,batch_en_att_2,batch_en_att_3,rs_12,rr_12,
                              rs_23,rr_23,self.r_type_12,self.r_type_23)
            pred  = torch.permute(pred, (1, 0, 2))
            logger_train.flush()
    ...
    def train(self, logger_train, run_ex_dir):
        ...
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(model_optim, 'min', factor=0.5, patience=10)
        ...
        for epoch in range(self.args.epochs):
            ...
            for i, (batch_en_att_1,batch_en_att_2,batch_en_att_3,batch_label) in enumerate(train_loader):
                iter_count += 1
    ...
    def test(self, logger_test,run_ex_dir):
        ...
        preds = []
        trues = []
        for i, (batch_en_att_1,batch_en_att_2,batch_en_att_3,batch_label) in enumerate(test_loader):
            pred = self.model(batch_en_att_1, batch_en_att_2, batch_en_att_3, rs_12, rr_12,
                              rs_23, rr_23,self.r_type_12,self.r_type_23)
            pred = torch.permute(pred, (1, 0, 2))
        ...
        logger_test.write("【预测结果展示（前10条轨迹）】" + '\n')
